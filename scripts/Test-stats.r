...
### helper variables 
setwd("C:/Users/jonas/OneDrive/Geowissenschaften/Master/2semester/Phylogenetics/project/analysis")
model <- "MkVP+G"
dataset <- "nudibranchia_characters.nex"
...
for (i in 1:length(sim_reps)){
  ...
  for (n in 1:2){
    if (n == 1){
      ...
      for (j in 1:length(TestStats[,1])){
        ...
        ES <- (empri - TestStats[j,"Retention Index"] ) / Std 
        Effect_Size <- rbind(Effect_Size, ES)
  ...
  #save p-values and effect sizes as csv
  write.csv(Results, "P-values_MKVP_G.csv")
